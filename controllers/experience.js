...
function postExperience (req, res) {
    ...
    const { job, city, country, startDate, finishDate, description, company, tags } = req.body;
    ...
    if (!job || !city || !country || !description || !company || !tags ) {
        console.log("job", job)
        res.status(403).send({code: 403, message: "Por favor, introduce todos los valores necesarios"})
    } else {
